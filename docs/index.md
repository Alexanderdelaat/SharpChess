SharpChess
EEN APPLICATIE GEMAAKT IN C#

LAAT, ALEX A. DE

SharpChess
SharpChess Architectuur-Overzicht
SharpChess
...
(^1) Onderhoudbaarheid Het project moet begrijpelijk en aanpasbaar blijven
tijdens ontwikkeling en beoordeling.
(^2) Correctheid Schaakregels, authenticatiestromen en
toestandsovergangen moeten consistent werken.
(^3) Beveiliging Wachtwoorden, tokens en beveiligde endpoints
...
(^5) Testbaarheid Belangrijke logica moet verifieerbaar zijn via
geautomatiseerde tests en API-tests.
(^6) Bruikbaarheid De interface moet kernacties zoals registratie,
